...
from .expr import Visitor, Statement, Expr, BlockStatement, VarStatement, \
    Variable, Assign, FunctionStatement, ExpressionStatement, IfStatement, \
    PrintStatement, ReturnStatement, WhileStatement, Binary, Call, Grouping, \
    Literal, Logical, Unary
...
class Resolver(Visitor):
    ...
    def visit_return_statement(self, statement: ReturnStatement) -> Any:
        if self.__current_function != FunctionKind.FUNCTION:
            Error.error_token(
                statement.keyword,
                "Can't return from a top-level code"
            )

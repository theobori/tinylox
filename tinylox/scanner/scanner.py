...
class Scanner:
    ...
    def __scan_token(self):
        ...
        match char:
            case "(":
                self.__add_token(TokenKind.LEFT_PAREN)
            case ")":
                self.__add_token(TokenKind.RIGHT_PAREN)
            case "{":
                self.__add_token(TokenKind.LEFT_BRACE)
            case "}":
                self.__add_token(TokenKind.RIGHT_BRACE)
            case ",":
                self.__add_token(TokenKind.COMMA)
            case ".":
                self.__add_token(TokenKind.DOT)
            case "-":
                self.__add_token(TokenKind.MINUS)
            case "+":
                self.__add_token(TokenKind.PLUS)
            case ";":
                self.__add_token(TokenKind.SEMICOLON)
            case "*":
                self.__add_token(TokenKind.STAR)
            case "!":
                self.__add_token(
                    TokenKind.BANG_EQUAL if self.__match("=") else TokenKind.BANG
                )
            case "=":
                self.__add_token(
                    TokenKind.EQUAL_EQUAL if self.__match("=") else TokenKind.EQUAL
                )
            case "<":
                self.__add_token(
                    TokenKind.LESS_EQUAL if self.__match("=") else TokenKind.LESS
                )
            case ">":
                self.__add_token(
                    TokenKind.GREATER_EQUAL if self.__match("=") else TokenKind.GREATER
                )
            case "\r" | "\t" | " ":
                pass
            case "\n":
